    ...
    pub fn ann_accuracy(&self, request: QueryRequest, response_to_validate: &QueryResponse) {
        ...
        for query_i in 0..request.embeddings.len() {
            ...
            for (i, received_id) in response_to_validate.ids[query_i].iter().enumerate() {
                ...
                let correct_distance =
                    (received_distance - expected_distance).abs() < accuracy_threshold;
                if was_unexpected {
                    if correct_distance {
                        missing -= 1;
                    } else {
                    ...
                    assert!(
                        correct_distance,
                        "Expected distance to be within {:.6} of {:.6}, was {:.6}.",
                        accuracy_threshold, expected_distance, received_distance
                    );
                }
            }
            ...
            println!(
                "difference: {:?}, size: {}, missing: {}",
                expected_ids.difference(&received_ids),
                size,
                missing
            );
            if size > 0 {
